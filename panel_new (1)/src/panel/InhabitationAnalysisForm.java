 ...
 * @author Дима
 */
public class InhabitationAnalysisForm extends javax.swing.JPanel {

    /**
     * Creates new form city_count
     */
    private static DefaultListModel listModeCity = new DefaultListModel();
    private static DefaultListModel listModelCountry = new DefaultListModel();

    public InhabitationAnalysisForm(Map<String, Integer> city, Map<String, Integer> country) throws IOException {
        initComponents();
        jList1.setModel(listModeCity);
        jList2.setModel(listModelCountry);
        listModeCity.removeAllElements();
        listModelCountry.removeAllElements();
        Calendar SaveData = Calendar.getInstance();
        baseСount(city, listModeCity, 1);
        vr(SaveData, "Закончили сбор городов ");

        SaveData = Calendar.getInstance();
        baseСount(country, listModelCountry, 2);
        vr(SaveData, "Закончили сбор стран ");

    }

    public InhabitationAnalysisForm(String city, String country) {
        initComponents();
    ...
    public void baseСount(Map<String, Integer> city, DefaultListModel listModelcityl, int select) throws IOException {
        int symm = 0;
        ...
        Map<String, Double> cityyper_res = new HashMap<String, Double>();
        for (String vl : cityyper.keySet()) {
            if (cityyper.get(vl) >= 0.2) {
                double vll = cityyper.get(vl);
               // int vlll = (int) vll;
                cityyper_res.put(vl, vll);
            }
        }
        Map<String, Double> cityyper_res_res = sortByValue(cityyper_res);
        int i = 0;
        for (String vl : cityyper_res_res.keySet()) {
            ...
            if (select == 1) {
                ...
                if (!cityty.equals("")) {
                    AnalysisPanel.cit += cityty + " - " + cityyper_res_res.get(vl) + "%" + ",";
            ...
            if (select == 2) {
                ...
                if (!country.equals("")) {
                    AnalysisPanel.countr += country + " - " + cityyper_res_res.get(vl) + "%" + ",";
    ...
    private String city(String ID) throws MalformedURLException, IOException {
    ...
    private String country(String ID) throws MalformedURLException, IOException {
